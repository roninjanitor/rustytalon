...
- **M4 done:** `stage_candidate`, `list_candidates`, `approve_candidate`, `reject_candidate`, `update_candidate` — candidates are stored as `:GraphCandidate` nodes in Neo4j itself (not a new `Database`-trait table). Review is available both through tool calls from any channel and through a dedicated web UI review list (F16, see §12) — the Discord-react-to-approve option remains unbuilt.
- **F8's "scheduled routine"** is now auto-seeded on boot (F11, see §12) rather than requiring the user to call `routine_create` manually, though the underlying mechanism is unchanged — it's still a normal `FullJob` routine (`description` tells the agent to review conversation history and call `stage_candidate`), reusing the general-purpose routine engine instead of adding graph-specific scheduling code.
  - **Correction:** an earlier revision of this document claimed this was already usable, but `RoutineAction::FullJob` execution (`src/agent/routine_engine.rs`) previously had no tool-calling loop at all — it silently fell back to a single bare LLM completion with zero tools available, meaning a `stage_candidate`-calling routine would have run on schedule and done nothing, with no error surfaced. This has since been fixed: `execute_full_job()` now runs a real multi-turn tool-calling loop (`Reasoning` + `ToolRegistry`, mirroring `Worker`'s pattern minus job-state-machine/audit bookkeeping that doesn't apply to unattended routine runs). Approval-gated tools are still blocked in this context (no human present to approve), consistent with how `Worker` handles autonomous jobs.
...
- **Visualization UI**: built, see F12/F13/F16 in §12.
...
|----|-------------|--------|
| F11 | A scheduled extraction routine actually exists and runs (F8 is implemented in principle but no routine has been created) — needed before any of the below can be evaluated with real data. | Done — auto-seeded on boot (see note below), no longer requires the user to remember to call `routine_create` |
...
| F14 | Ambient graph context injection: before an LLM turn, detect likely entity mentions in the incoming message and pull relevant `get_entity_context` results into the prompt automatically, rather than relying solely on the model's own tool-selection judgement. Must be introduced only after F11 has run long enough to validate extraction quality — ambient injection of untrustworthy graph data is worse than no graph context at all. | Not started, blocked on F11 data. F11 now runs automatically, but no real usage window has accrued yet — the first real batch of candidates staged under F11 (9 candidates, 3 differently-named duplicates of the same project) is itself evidence extraction quality isn't validated yet, so this stays blocked. |
| F15 (stretch) | Confidence-scored auto-commit for high-confidence candidates (the PRD's original Phase C), informed by real approve/reject ratios collected from F11's review process. | Not started, blocked on F11 data, same reasoning as F14 |
| F16 | A web UI review list for staged candidates (approve/reject/edit), rather than tool-calls-only — the open question left unresolved in the M4 note below. | Done — see note below |

Recommended build order: F11 → F16 → F12 → F13 → F14 → F15. F16 (review UI) was pulled forward ahead of F14/F15 for the same reason F12 was: a small/noisy graph is safe to browse and review by hand, and doing so surfaced the extraction-quality problem (naming inconsistency across passes) that F14/F15 are explicitly gated on.

**F11/F16 implementation notes:** `src/main.rs` seeds a routine named "Knowledge graph extraction" on boot (cron `0 */6 * * *`, `FullJob` action) if Neo4j is connected and no routine by that name exists yet for the default user — idempotent, and never touches the routine again once created, so user edits/disables/deletes persist across restarts. For F16, `GraphClient` gained `update_candidate()` (`src/graph/client.rs`) to edit a pending candidate's staged entities/relationships before approval, and the web gateway gained four endpoints under `/api/graph/candidates*` (list, edit, approve, reject). The Graph tab's "Pending review" panel (`static/index.html`/`app.js`/`style.css`) lists candidates with an editable name field per staged entity — the concrete motivating case was three staged candidates naming the same real-world project three different ways across extraction passes; renaming the non-canonical ones to match before approving lets `create_entity`'s existing `MERGE` collapse them into one node instead of creating duplicates, rather than requiring a separate merge-after-the-fact step.
