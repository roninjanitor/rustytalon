...
- RustyTalon installed and configured (`rustytalon onboard`)
...
rustytalon pairing list telegram

# List as JSON
rustytalon pairing list telegram --json

# Approve a user by code
rustytalon pairing approve telegram ABC12345
```

### Configuration

Edit `~/.rustytalon/channels/telegram.capabilities.json` (or the config injected by the host):
...
- **Secrets store**: `rustytalon` CLI (if available)
...
- For webhook mode: Start your tunnel before `rustytalon run`
