...
This guide covers how to build WASM channel modules for RustyTalon.
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for RustyTalon"
...
**Do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. RustyTalon builds channels from source:
...
# Install (or use rustytalon onboard to install bundled channel)
mkdir -p ~/.rustytalon/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.rustytalon/channels/
```
...
# Deploy to ~/.rustytalon/channels/
cp target/wasm32-wasip2/release/my_channel.wasm ~/.rustytalon/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.rustytalon/channels/
```
