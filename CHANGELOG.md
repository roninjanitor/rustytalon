...
### Added
- **Knowledge graph candidate review UI + auto-seeded extraction routine** — the graph's staged-extraction review (`stage_candidate`/`approve_candidate`/`reject_candidate`) previously required chatting with the agent to do anything, and the F8 extraction routine had to be created manually via `routine_create` or the graph stayed empty forever. Added a "Pending review" panel to the web gateway's Graph tab (`/api/graph/candidates*` endpoints, `GraphClient::update_candidate`) that lists staged candidates and lets you approve, reject, or edit a staged entity's name before approving — the last one matters because extraction runs sometimes name the same real-world thing slightly differently pass to pass (e.g. "Project X" vs "Project X Extension"); renaming to a canonical name before approving lets `create_entity`'s existing `MERGE` collapse them into one node instead of creating duplicates. Also, on boot, a "Knowledge graph extraction" routine (cron, every 6h) is now created automatically if Neo4j is connected and no such routine exists yet, so ingestion starts without the user having to remember to ask for it; idempotent and never overwrites a routine the user has since edited or disabled.
...
- **`search_context` tool ties workspace memory and the knowledge graph together, and routines can no longer pollute daily logs** — the auto-seeded "Knowledge graph extraction" routine ran through `execute_full_job`, which handed every `FullJob` routine the *entire* global tool registry regardless of what its prompt asked it to use; despite being instructed to only call `stage_candidate`, its LLM would call `memory_write` and narrate its own run summaries (`"[Routine: graph-extraction] Checked daily logs... nothing to stage"`) straight into the same `daily/*.md` file real chat gets logged to, showing up as noise in every later `memory_search`/`memory_read`. `RoutineAction::FullJob` gained an optional `tool_allowlist: Vec<String>` field, enforced both when building the tool definitions the LLM sees (`ToolRegistry::tool_definitions_for`) and again when a tool call actually executes (`execute_tool_for_routine` now rejects any call outside the allowlist) — closing the gap where a model could still call an unlisted tool by name. The seeded extraction routine now sets this to graph-only tools, so it's physically unable to write to memory. Separately, memory (unstructured chat/notes) and the graph (structured entities/relationships) had no code linkage — a new `search_context` tool (feature-gated behind `neo4j`, registered alongside `register_memory_tools`/`register_graph_tools`) fans a query out to both `Workspace::search` and `GraphClient::search_entities` in parallel and returns merged, labeled results in one call, so relationship-flavored questions don't depend on the model remembering to call two separate tools.
