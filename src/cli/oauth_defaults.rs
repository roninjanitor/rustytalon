...
//!
//! - **Compile time**: Set RUSTYTALON_GOOGLE_CLIENT_ID / RUSTYTALON_GOOGLE_CLIENT_SECRET
...
const GOOGLE_CLIENT_ID: &str = match option_env!("RUSTYTALON_GOOGLE_CLIENT_ID") {
    Some(v) => v,
    None => "564604149681-efo25d43rs85v0tibdepsmdv5dsrhhr0.apps.googleusercontent.com",
};
const GOOGLE_CLIENT_SECRET: &str = match option_env!("RUSTYTALON_GOOGLE_CLIENT_SECRET") {
    Some(v) => v,
...
<title>RustyTalon - {heading}</title>
<style>
...
<body>
  <div class="card">
    ...
    <h1>{heading}</h1>
    <p>{subtitle}</p>
    <div class="brand">RustyTalon</div>
  </div>
</body>
</html>"#,
        heading = heading,
...
mod tests {
    ...
    fn test_landing_html_success_contains_key_elements() {
        ...
        assert!(html.contains("RustyTalon"));
