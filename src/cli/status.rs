...
pub async fn run_status_command() -> anyhow::Result<()> {
    ...
    // LLM Provider
    print!("  LLM:         ");
    let backend = std::env::var("LLM_BACKEND").unwrap_or_else(|_| "anthropic".to_string());
    let has_key = match backend.as_str() {
        "anthropic" => std::env::var("ANTHROPIC_API_KEY").is_ok(),
        "openai" => std::env::var("OPENAI_API_KEY").is_ok(),
        "ollama" => true, // No key needed
        _ => std::env::var("LLM_API_KEY").is_ok(),
    };
    if has_key {
        println!("configured ({})", backend);
    } else {
        println!("not configured (set API key for {})", backend);
    }
    ...
    println!(
        "\n  Config:      {}",
        crate::bootstrap::rustytalon_env_path().display()
    );
