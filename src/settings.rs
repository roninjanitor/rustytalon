...
//!
//! Stores user preferences in ~/.rustytalon/settings.json.
...
pub struct Settings {
    ...
    // === Step 3: Auth (legacy session.json migrated to DB) ===
...
fn default_sandbox_image() -> String {
    "rustytalon-worker:latest".to_string()
}
...
impl Settings {
    ...
    /// Get the default settings file path (~/.rustytalon/settings.json).
...
mod tests {
    ...
    fn test_get_setting() {
        ...
        assert_eq!(settings.get("agent.name"), Some("rustytalon".to_string()));
        assert_eq!(
    ...
    fn test_reset_setting() {
        ...
        assert_eq!(settings.agent.name, "rustytalon");
    }
