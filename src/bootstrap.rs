//! Bootstrap helpers for RustyTalon.
//!
...
//!
//! File: `~/.rustytalon/.env` (standard dotenvy format)

use std::path::PathBuf;

/// Path to the RustyTalon-specific `.env` file: `~/.rustytalon/.env`.
pub fn rustytalon_env_path() -> PathBuf {
    dirs::home_dir()
...
/// Load env vars from `~/.rustytalon/.env` (in addition to the standard `.env`).
///
/// Call this **after** `dotenvy::dotenv()` so that the standard `./.env`
/// takes priority over `~/.rustytalon/.env`. dotenvy never overwrites
/// existing env vars, so the effective priority is:
///
///   explicit env vars > `./.env` > `~/.rustytalon/.env`
///
/// If `~/.rustytalon/.env` doesn't exist but the legacy `bootstrap.json` does,
/// extracts `DATABASE_URL` from it and writes the `.env` file (one-time
/// upgrade from the old config format).
pub fn load_rustytalon_env() {
    let path = rustytalon_env_path();
...
fn migrate_bootstrap_json_to_env(env_path: &std::path::Path) {
    let rustytalon_dir = env_path
        .parent()
        .unwrap_or_else(|| std::path::Path::new("."));
    let bootstrap_path = rustytalon_dir.join("bootstrap.json");
...
/// Write `DATABASE_URL` to `~/.rustytalon/.env`.
///
...
/// One-time migration of legacy `~/.rustytalon/settings.json` into the database.
///
...
pub async fn migrate_disk_to_db(
    ...
    let rustytalon_dir = dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
        .join(".rustytalon");
    let legacy_settings_path = rustytalon_dir.join("settings.json");
    ...
    let old_bootstrap = rustytalon_dir.join("bootstrap.json");
...
mod tests {
    ...
    fn test_save_and_load_database_url() {
        ...
        let url = "postgres://localhost:5432/rustytalon_test";
        ...
        assert_eq!(
            content,
            "DATABASE_URL=\"postgres://localhost:5432/rustytalon_test\"\n"
        );
    ...
    fn test_save_database_url_with_hash_in_password() {
        ...
        let url = "postgres://user:p%23ss@localhost:5432/rustytalon";
        std::fs::write(&env_path, format!("DATABASE_URL=\"{}\"\n", url)).unwrap();
    ...
    #[test]
    fn test_rustytalon_env_path() {
        let path = rustytalon_env_path();
        assert!(path.ends_with(".rustytalon/.env"));
    }
    ...
    fn test_migrate_bootstrap_json_to_env() {
        ...
        let bootstrap_json = serde_json::json!({
            "database_url": "postgres://localhost/rustytalon_upgrade",
