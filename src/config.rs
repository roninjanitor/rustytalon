//! Configuration for RustyTalon.
//!
//! Settings are loaded with priority: env var > database > default.
//! `DATABASE_URL` lives in `~/.rustytalon/.env` (loaded via dotenvy early
...
impl Config {
    ...
    pub async fn from_db(
        ...
        crate::bootstrap::load_rustytalon_env();
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.rustytalon/.env`
...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.rustytalon/rustytalon.db).
...
impl DatabaseConfig {
    fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.rustytalon/.env via dotenvy early in startup.
...
/// Default libSQL database path (~/.rustytalon/rustytalon.db).
...
impl std::str::FromStr for LlmBackend {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(format!(
                "invalid LLM backend '{}', expected one of: anthropic, openai, ollama, openai_compatible",
                s
            )),
        }
    }
...
/// by setting `LLM_BACKEND` (e.g. `openai`, `ollama`, `openai_compatible`).
#[derive(Debug, Clone)]
...
pub struct ChannelsConfig {
    ...
    /// Directory containing WASM channel modules (default: ~/.rustytalon/channels/).
...
/// Get the default channels directory (~/.rustytalon/channels/).
...
pub struct WasmConfig {
    ...
    pub enabled: bool,
    /// Directory containing installed WASM tools (default: ~/.rustytalon/tools/).
    ...
    async fn resolve() -> Result<Self, ConfigError> {
        ...
        let (master_key, source) = if let Some(env_key) = optional_env("SECRETS_MASTER_KEY")? {
            ...
            match crate::secrets::keychain::get_master_key().await {
                Ok(key_bytes) => {
                    tracing::debug!("Master key found in OS keychain ({} bytes)", key_bytes.len());
...
/// Get the default tools directory (~/.rustytalon/tools/).
