...
- ✅ **Knowledge graph extraction folded into heartbeat consolidation (PRD F11, superseding the standalone routine)** - F11 was originally implemented as a separate auto-seeded `FullJob` routine on a 6h cron reading recent daily logs and calling `stage_candidate`. That routine ran uncoordinated with heartbeat's memory consolidation (`consolidate_daily_logs`, `src/agent/heartbeat.rs`), which also mines daily logs and -- critically -- deletes each log right after processing it once the day rolls over. Because the two passes weren't synchronized, late-day content could be consolidated-and-deleted before the 6h-cron routine ever got to read it, silently losing graph candidates. Graph staging is now done directly inside `consolidate_daily_logs`'s existing LLM call (same call that extracts `USER:`/`MEMORY:` facts): if a `GraphClient` is attached (`HeartbeatRunner::with_graph_client`, wired from `AgentDeps.graph_client` in `main.rs`), the prompt also asks for an optional `GRAPH_CANDIDATES:` JSON block (parsed by `parse_graph_candidates`), staged via `GraphClient::stage_candidate` in the same pass -- guaranteeing by construction that staging happens before deletion, and halving LLM calls per day versus two separate passes. The standalone extraction routine and its boot-time auto-seeding are removed.
...
2. **Memory consolidation** — for each daily log older than today, makes a lightweight LLM call to extract atomic facts (`USER:` / `MEMORY:` lines) into `USER.md` and `MEMORY.md`, then deletes the raw log. If a knowledge-graph client is attached (`neo4j` feature, Neo4j connected), the same LLM call also proposes graph candidates (`GRAPH_CANDIDATES:` block) staged via `GraphClient::stage_candidate` before the log is deleted — see the Knowledge Graph section below.
...
**F8** ("a scheduled routine periodically reviews recent conversation history") is satisfied by heartbeat's memory consolidation, not a dedicated routine — see the Heartbeat System section above. Consolidation already reads every daily log exactly once (right before deleting it), so folding graph-candidate staging into that same LLM call gives F8 a natural, race-free cadence at no extra cost. An earlier version of this used a separate auto-seeded `FullJob` routine on a 6h cron instead; it was retired because it read the same daily logs independently of consolidation's deletion, creating a window where late-day content could be deleted before the routine ever saw it. You can still create a custom routine via `routine_create` for other extraction cadences (e.g. reviewing sources other than daily logs), but it's no longer needed for the basic "keep the graph in sync with conversation" case.
