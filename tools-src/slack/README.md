...
A standalone WASM component that provides Slack integration for RustyTalon. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.rustytalon/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.rustytalon/tools/slack.wasm
cp slack.capabilities.json ~/.rustytalon/tools/
```
...
```bash
rustytalon secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
rustytalon secret set slack_bot_token "xoxb-..."
```
