...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter("rustytalon=debug")
        .init();
    ...
    let hb_config = rustytalon::agent::HeartbeatConfig::default();
    ...
    println!("=== Result ===\n");
    match &result {
        rustytalon::agent::HeartbeatResult::Ok => {
            println!("HeartbeatResult::Ok");
            println!("  LLM responded HEARTBEAT_OK, nothing needs attention.");
        }
        rustytalon::agent::HeartbeatResult::NeedsAttention(msg) => {
            println!("HeartbeatResult::NeedsAttention");
            println!("  Message:\n{}", msg);
        }
        rustytalon::agent::HeartbeatResult::Skipped => {
        ...
        rustytalon::agent::HeartbeatResult::Failed(err) => {
