...
use rustytalon::channels::web::server::{GatewayState, start_server};
use rustytalon::channels::web::sse::SseManager;
use rustytalon::channels::web::ws::WsConnectionTracker;
use rustytalon::error::LlmError;
use rustytalon::llm::{
...
impl LlmProvider for MockLlmProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let user_msg = req
            ...
            .iter()
            .rev()
            .find(|m| m.role == rustytalon::llm::Role::User)
    ...
    async fn complete_with_tools(
        ...
        if let Some(tool) = req.tools.first() {
            Ok(ToolCompletionResponse {
                content: None,
                tool_calls: vec![rustytalon::llm::ToolCall {
...
async fn test_chat_completions_streaming() {
    ...
    assert_eq!(
        resp.headers()
            .get("x-rustytalon-streaming")
            .and_then(|v| v.to_str().ok()),
        Some("simulated"),
        "Expected x-rustytalon-streaming: simulated header"
    );
